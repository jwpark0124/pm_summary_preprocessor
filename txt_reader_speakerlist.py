# -*- coding: utf-8 -*-
"""txt_reader_speakerlist.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/13s6PGAqEI_rq36wxl9bdNY-EPa3HhaLJ
"""
...
sample = sorted(
    glob("/Users/jaewanpark/Documents/회의록/pm_summary_preprocessor(pjw)/*.txt"))
# pprint(sample)
...
def clean_up_or(txt):
    ...
    for han in range(len(ttt)):
        # ***
        ktext = ttt[han].replace("金", a).replace("金", a).replace("樂", b).replace(
            "樂", b).replace("柳", c).replace("柳", c).replace("劉", d).replace("劉", d)
...
def dialog_extractor(text):
    ...
    for sid in range(len(text)):
        if sid >= s_start and sid <= s_end - 1:
            sent = text[sid].lstrip()
            sent = hanja.translate(sent, 'substitution').replace("金", "김").replace("李", "이").replace("梁", "양").replace("羅", "나").replace("利", "이").replace("勞", "노").replace(
                "樂", "락").replace("盧", "노").replace("樂", "락").replace("龍", "용").replace("沈", "심").replace("呂", "려").replace("寧", "영").replace("宅", "택").replace("劉", "유")
...
speaker_list2__2 = speaker_extractor2_or(
    text, speaker_list, speaker_list__, speaker_ex)
...
korea_formatted_data = convert_to_korea(
    speaker_list__, speaker_list2__2, dialog_json, id)
...
def speaker_id_json(id):
    ...
    k = 0
    files = sorted(glob(
        "/Users/jaewanpark/Documents/회의록/pm_summary_preprocessor(pjw)/요약대상회의록-2차/*.txt"))
    for fname in files:
        with open(fname, 'r', encoding='utf-8-sig') as file:
            txt = file.readlines()
            pass
            ...
            dd.append(d)
    with open("/Users/jaewanpark/Documents/회의록/pm_summary_preprocessor(pjw)/txt_reader_speakerlist/2차-speakerlist.json", 'w') as f:
